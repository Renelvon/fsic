...
class NFSIC(IndTest):
    ...
    def __init__(
        self, k, l, V, W, alpha=0.01, reg="auto", n_permute=None, seed=87
    ):
        """
    ...
    def list_permute(X, Y, k, l, V, W, n_permute=400, seed=873, reg=0):
        ...
        if V.shape[0] != W.shape[0]:
            raise ValueError(
                "Number of locations in V, W (rows) must be the same."
            )
        n = X.shape[0]
    ...
    def _list_permute_naive(X, Y, k, l, V, W, n_permute=400, seed=873, reg=0):
        ...
        arr_nfsic = permute_null_dist(
            pdata, nfsic, n_permute=n_permute, seed=seed
        )
        return arr_nfsic
...
class GaussNFSIC(NFSIC):
    ...
    def __init__(
        ...
        super(GaussNFSIC, self).__init__(
            k, l, V, W, alpha=alpha, reg=reg, n_permute=n_permute, seed=seed
        )
    ...
    def optimize_locs_widths(
        ...
        V, W = GaussNFSIC.init_check_subset(
            pdata, J, k, l, n_cand=20, subsample=2000, seed=seed + 27
        )
        ...
        bestij, _ = GaussNFSIC.grid_search_gwidth(
            pdata, V, W, list_gwidthx, list_gwidthy
        )
        gwidthx0 = list_gwidthx[bestij[0]]
        gwidthy0 = list_gwidthy[bestij[1]]
        assert util.is_real_num(gwidthx0), "gwidthx0 not real. Was %s" % str(
            gwidthx0
        )
        assert util.is_real_num(gwidthy0), "gwidthy0 not real. Was %s" % str(
            gwidthy0
        )
        ...
        V, W, gwidthx, gwidthy, info = generic_optimize_locs_widths(
            pdata,
            V,
            W,
            gwidthx0,
            ...
            reg=reg,
            seed=seed + 1,
            gwidthx_lb=gwidthx_lb,
            gwidthx_ub=gwidthx_ub,
            gwidthy_lb=gwidthy_lb,
            gwidthy_ub=gwidthy_ub,
        )
        assert util.is_real_num(gwidthx), "gwidthx is not real. Was %s" % str(
            gwidthx
        )
        assert util.is_real_num(gwidthy), "gwidthy is not real. Was %s" % str(
            gwidthy
        )
    ...
    def grid_search_gwidth(pdata, V, W, list_gwidthx, list_gwidthy):
        ...
        besti, objs = nfsic_grid_search_kernel(
            pdata, V, W, list_gauss_kernelx, list_gauss_kernely
        )
        return besti, objs
    ...
    def init_check_subset(
        ...
        for i in range(n_cand):
            V, W = GaussNFSIC.init_locs_joint_subset(
                pdata, n_test_locs, seed=seed_seq_joint[i]
            )
            if subsample < n:
                I = util.subsample_ind(
                    n, n_test_locs, seed=seed_seq_joint[i] + 1
                )
                XI = X[I, :]
                YI = Y[I, :]
            else:
        ...
        seed_seq_prod = util.subsample_ind(
            5 * n_cand, n_cand, seed=seed * 3 + 1
        )
        for i in range(n_cand):
            V, W = GaussNFSIC.init_locs_marginals_subset(
                pdata, n_test_locs, seed=seed_seq_prod[i]
            )
            if subsample < n:
                I = util.subsample_ind(
                    n, n_test_locs, seed=seed_seq_prod[i] + 1
                )
                XI = X[I, :]
                YI = Y[I, :]
            else:
        ...
        if objs_joint[bind_joint] >= objs_prod[bind_prod]:
            V, W = GaussNFSIC.init_locs_joint_subset(
                pdata, n_test_locs, seed=seed_seq_joint[bind_joint]
            )
        else:
            V, W = GaussNFSIC.init_locs_marginals_subset(
                pdata, n_test_locs, seed=seed_seq_prod[bind_prod]
            )
        return V, W
...
def generic_optimize_locs_widths(
    ...
    s = func_obj(
        ...
        Vth,
        Wth,
        tensor.sqr(gwidthx_sq_th),
        tensor.sqr(gwidthy_sq_th),
        regth,
        X.shape[0],
        V0.shape[0],
    )

    g_V, g_W, g_gwidthx_sq, g_gwidthy_sq = tensor.grad(
        s, [Vth, Wth, gwidthx_sq_th, gwidthy_sq_th]
    )
    ...
    functh = theano.function(
        ...
        updates=[
            (
                Vth,
                Vth
                + V_step * g_V / it ** step_pow / tensor.sum(g_V ** 2) ** 0.5,
            ),
            (
                Wth,
                Wth
                + W_step * g_W / it ** step_pow / tensor.sum(g_W ** 2) ** 0.5,
            ),
            (it, it + 1),
            ...
            (
                ...
                gwidth_constrain(
                    gwidthx_sq_th
                    + gwidthx_step
                    * tensor.sgn(g_gwidthx_sq)
                    * tensor.minimum(
                        tensor.abs_(g_gwidthx_sq), max_gwidthx_step
                    )
                    / it ** step_pow,
                    tensor.sqrt(gwidthx_lb),
                    tensor.sqrt(gwidthx_ub),
                ),
            ),
            (
                gwidthy_sq_th,
                gwidth_constrain(
                    gwidthy_sq_th
                    + gwidthy_step
                    * tensor.sgn(g_gwidthy_sq)
                    * tensor.minimum(
                        tensor.abs_(g_gwidthy_sq), max_gwidthy_step
                    )
                    / it ** step_pow,
                    tensor.sqrt(gwidthy_lb),
                    tensor.sqrt(gwidthy_ub),
                ),
            ),
        ],
    )
    ...
    with util.NumpySeedContext(seed=seed):
        for t in range(max_iter):
            ...
            ind = np.random.choice(
                n, min(int(batch_proportion * n), n), replace=False
            )
    ...
    if t >= 0:
        ...
        opt_gwidthx = (
            gwidthxs[-1] if util.is_real_num(gwidthxs[-1]) else gwidthx0
        )
        opt_gwidthy = (
            gwidthys[-1] if util.is_real_num(gwidthys[-1]) else gwidthy0
        )
...
class FiniteFeatureHSIC(IndTest):
    ...
    def list_permute_spectral(
        ...
        if features_x.shape[0] != features_y.shape[0]:
            raise ValueError(
                "features_x, features_y must have the same number of rows n."
            )
        ...
        sim_hsics = FiniteFeatureHSIC.simulate_null_dist(
            eigx, eigy, biased_hsic=True, n_simulate=n_simulate, seed=seed
        )
        return sim_hsics, eigx, eigy

    @staticmethod
    def simulate_null_dist(
...
class NystromHSIC(FiniteFeatureHSIC):
    ...
    def __init__(
        self, k, l, induce_x, induce_y, n_simulate=5000, alpha=0.01, seed=92
    ):
        """
        k: kernel for X
        l: kernel for Y
...
class RDC(IndTest):
    ...
    def compute_stat(self, pdata, return_eigvals=False):
        ...
        bartlett_stat = -(
            n - 1 - 0.5 * (Xrff.shape[1] + Yrff.shape[1] + 1)
        ) * np.sum(np.log(1 - evals[:minD] ** 2))
...
class RDCPerm(IndTest):
    ...
    @staticmethod
    def list_permute(
        X, Y, fmx, fmy, n_permute=400, seed=8273, use_copula=True, cca_reg=1e-5
    ):
        """
    ...
    @staticmethod
    def _list_permute_naive(
        X, Y, fmx, fmy, n_permute=400, seed=8273, use_copula=True
    ):
        """
        Repeatedly mix, permute X, Y so that pairs are broken, and compute RDC.
        This is intended to be used to approximate the null distribution.
